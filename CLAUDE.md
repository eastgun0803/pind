...
| Phase | 상태 | 브랜치 | 주요 산출물 |
|-------|------|--------|------------|
| 0. 부트스트랩 | ✅ 완료 | main | 모노레포, pre-commit, Makefile |
| 1. DB & DTO | ✅ 완료 | feat/phase-1 | Video/Place 모델, Alembic 마이그레이션, Pydantic 스키마, mock 라우터 |
| 2. Frontend 뼈대 | ✅ 완료 | feat/phase-2 | Auth, VideoForm, Leaflet 지도, mock API 연동 |
| 3. AI 파이프라인 | ✅ 완료 | feat/phase-3-4 | Gemini Files API, yt-dlp, Google Places, sentence-transformers dedup |
| 4. Realtime & UI | ✅ 완료 | feat/phase-3-4 | Supabase Realtime, YouTube iframe 팝업, Extension popup + content script |
| 5. 보안 & 배포 | ⬜ 미시작 | — | RLS 검증, Render/Vercel 배포 |

> 상세 진행 내역은 `PROGRESS.md` 참고.

---

## Gemini 모델 정책

- **현재 사용 모델**: `gemini-2.5-flash` (stable GA) — `GEMINI_MODEL` 환경변수로 관리
- **절대 사용 금지**: `gemini-2.0-*` (2026-06-01 shutdown), `gemini-1.5-*` (이미 404)
- **Gemini 3 시리즈**: 대부분 preview — GA 전환 전까지 사용 금지
- 자세한 내용: `apps/api/CLAUDE.md` Gemini 모델 버전 섹션 참고
